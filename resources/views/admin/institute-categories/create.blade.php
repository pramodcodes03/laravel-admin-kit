<x-layout.admin title="Add Institute Category">
    <div>
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Add Institute Category</h5>
        ...
        <div class="panel">
            ...
            <form action="{{ route('admin.institute-categories.store') }}" method="POST" x-data="categoryForm()">
                @csrf
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="slug" name="slug" type="text" class="form-input bg-gray-100 dark:bg-gray-800"
                            :value="slug" readonly />
                    ...
                    <div>
                        ...
                        <input id="icon" name="icon" type="text" class="form-input" value="{{ old('icon') }}" placeholder="e.g. computer, school, fitness" />
                        <p class="text-xs text-gray-400 mt-1">Icon identifier (text/class name)</p>
                    </div>
                    <div>
                        <label for="sort_order">Sort Order</label>
                        <input id="sort_order" name="sort_order" type="number" class="form-input" value="{{ old('sort_order', 0) }}" min="0" />
                    </div>
                    <div>
                        <label for="color">Background Color</label>
                        <div class="flex gap-2">
                            <input id="color" name="color" type="color" class="form-input w-12 h-10 p-1 cursor-pointer" value="{{ old('color', '#3B82F6') }}" />
                            <input type="text" class="form-input flex-1" x-ref="colorText" value="{{ old('color', '#3B82F6') }}"
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input id="text_color" name="text_color" type="color" class="form-input w-12 h-10 p-1 cursor-pointer" value="{{ old('text_color', '#ffffff') }}" />
                            <input type="text" class="form-input flex-1" value="{{ old('text_color', '#ffffff') }}"
    ...
    <script>
        function categoryForm() {
            return {
                slug: '{{ old('name') ? Str::slug(old('name')) : '' }}'
            };
        }
    </script>
</x-layout.admin>
