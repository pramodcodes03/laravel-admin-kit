<x-layout.admin title="Institute Categories">
    <div x-data="categoryList">
        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Institute Categories</h5>
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <a href="{{ route('admin.institute-categories.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
        ...
        @endif
        @if(session('error'))
            <div class="alert alert-danger flex items-center p-3.5 rounded-lg mb-4">
                ...
                <table class="table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2">Institutes</th>
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2 text-gray-500 text-sm" x-text="item.slug"></td>
                                <td class="px-4 py-2 text-sm" x-text="item.icon || '-'"></td>
                                <td class="px-4 py-2">
                                    <template x-if="item.color">
                                        <span class="inline-flex items-center gap-1">
                                            <span class="w-4 h-4 rounded-full border border-gray-200" :style="`background:${item.color}`"></span>
                                            <span class="text-xs" x-text="item.color"></span>
                                        </span>
                                    </template>
                                    <template x-if="!item.color"><span>-</span></template>
                                </td>
                                <td class="px-4 py-2" x-text="item.sort_order"></td>
                                <td class="px-4 py-2" x-text="item.institutes_count ?? '-'"></td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/institute-categories') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary">Edit</a>
                        ...
                        <tr x-show="items.length === 0">
                            <td colspan="9" class="px-4 py-4 text-center text-gray-500">No categories found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('categoryList', () => ({
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.institute-categories.index') }}?page=${page}`;
                ...
                toggleStatus(id) {
                    fetch(`{{ url('admin/institute-categories') }}/${id}/toggle-status`, {
                        method: 'PATCH',
                ...
                deleteItem(id) {
                    ...
                    swal.fire({ title: 'Are you sure?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/institute-categories') }}/${id}`, {
                                method: 'DELETE',
                                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                            }).then(res => res.json()).then(data => {
                                if (data.success) { this.showMessage(data.message); this.fetchData(this.pagination.current_page); }
