<x-layout.admin title="Edit Blog Post">
    <div>
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Edit Post</h5>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                    <div class="md:col-span-2 space-y-5">
                        <div>
                            ...
                            <input id="title" name="title" type="text" class="form-input" value="{{ old('title', $post->title) }}" required />
                        </div>
                        <div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            @if($post->cover_image)
                                <div class="mb-2">
                                    <img src="{{ \App\Helpers\StorageHelper::url($post->cover_image) }}" alt="Cover" class="h-20 w-auto object-cover rounded border border-gray-200" />
                            ...
                            <p class="text-xs text-gray-400 mt-1">Upload new to replace. Max 3MB.</p>
                        </div>
                        <div>
                            <label for="read_time">Read Time (minutes)</label>
                            <input id="read_time" name="read_time" type="number" class="form-input" value="{{ old('read_time', $post->read_time) }}" min="1" />
                        </div>
                        <div>
                            <label for="tags">Tags</label>
                            <input id="tags" name="tags" type="text" class="form-input"
                                value="{{ old('tags', $post->tags ? implode(', ', $post->tags) : '') }}"
                                placeholder="career, tips, courses" />
                            <p class="text-xs text-gray-400 mt-1">Comma-separated</p>
                        </div>
                        @if($post->published_at)
                            <div>
                                <p class="text-xs text-gray-500 uppercase tracking-wider">Published</p>
                                <p class="text-sm mt-1">{{ $post->published_at->format('d M Y, h:i A') }}</p>
                            </div>
                        @endif
