<x-layout.admin title="Blog Posts">
    <div x-data="blogList">
        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Blog Posts</h5>
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <button type="button" class="btn btn-outline-danger btn-sm"
                    x-show="searchText || filterStatus"
                    @click="clearFilters()">Clear</button>
                <a href="{{ route('admin.blog.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
                ...
                <table class="table-hover">
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    <div class="text-xs text-gray-400" x-text="(item.read_time || 3) + ' min read'"></div>
                                </td>
                                <td class="px-4 py-2 text-sm" x-text="item.admin ? item.admin.name : '-'"></td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <span class="badge" :class="item.is_featured ? 'bg-warning' : 'bg-gray-300 dark:bg-gray-600'" x-text="item.is_featured ? 'Featured' : 'Normal'"></span>
                                </td>
                                <td class="px-4 py-2 text-xs text-gray-500">
                                    <span x-text="item.published_at ? new Date(item.published_at).toLocaleDateString('en-IN') : '-'"></span>
                                </td>
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/blog') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary">Edit</a>
                        ...
                        <tr x-show="items.length === 0">
                            <td colspan="7" class="px-4 py-4 text-center text-gray-500">No blog posts found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('blogList', () => ({
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.blog.index') }}?page=${page}`;
                ...
                toggleStatus(id) {
                    fetch(`{{ url('admin/blog') }}/${id}/toggle-status`, {
                        method: 'PATCH',
                ...
                deleteItem(id) {
                    ...
                    swal.fire({ title: 'Delete post?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
