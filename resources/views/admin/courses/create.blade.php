<x-layout.admin title="Add Course">
    <div>
        <div class="flex items-center justify-between mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Add Course</h5>
            ...
            <form action="{{ route('admin.courses.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2 mb-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="short_description" name="short_description" class="form-textarea" rows="2">{{ old('short_description') }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label for="long_description">Full Description</label>
                        <textarea id="long_description" name="long_description" class="form-textarea" rows="6">{{ old('long_description') }}</textarea>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3 mb-6">
                    <div>
                        ...
                        <input id="duration_weeks" name="duration_weeks" type="number" class="form-input" value="{{ old('duration_weeks', 0) }}" min="0" />
                    </div>
                    <div>
                        <label for="hours_per_week">Hours per Week</label>
                        <input id="hours_per_week" name="hours_per_week" type="number" class="form-input" value="{{ old('hours_per_week', 0) }}" min="0" />
                    </div>
                    <div>
                        <label for="level">Level <span class="text-danger">*</span></label>
                        <select id="level" name="level" class="form-select" required>
                            @foreach(['Beginner','Intermediate','Advanced','All Levels'] as $lvl)
                                <option value="{{ $lvl }}" {{ old('level','Beginner') === $lvl ? 'selected' : '' }}>{{ $lvl }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                            ...
                            @foreach(['Offline','Online','Hybrid'] as $mode)
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="mode[]" value="{{ $mode }}" class="form-checkbox"
                                        {{ in_array($mode, old('mode', [])) ? 'checked' : '' }} />
                                    <span>{{ $mode }}</span>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3 mb-6">
                    <div>
                        ...
                        <input id="price_inr" name="price_inr" type="number" class="form-input" value="{{ old('price_inr', 0) }}" min="0" required />
                    </div>
                    <div>
                        <label for="original_price_inr">Original Price (INR)</label>
                        <input id="original_price_inr" name="original_price_inr" type="number" class="form-input" value="{{ old('original_price_inr') }}" min="0" placeholder="Strike-through price" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3 mb-6">
                    <div class="flex items-center gap-3">
                        <label class="relative inline-flex cursor-pointer">
                            <input type="checkbox" name="emi_available" value="1" class="sr-only peer" {{ old('emi_available') ? 'checked' : '' }} />
                    ...
                    <div class="flex items-center gap-3">
                        <label class="relative inline-flex cursor-pointer">
                            <input type="checkbox" name="certificate" value="1" class="sr-only peer" {{ old('certificate', true) ? 'checked' : '' }} />
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-primary"></div>
                        </label>
                    ...
                    <div class="flex items-center gap-3">
                        <label class="relative inline-flex cursor-pointer">
                            <input type="checkbox" name="placement_support" value="1" class="sr-only peer" {{ old('placement_support') ? 'checked' : '' }} />
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-primary"></div>
                        </label>
