<x-layout.admin title="Listing Requests">
    <div x-data="listingList">
        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Listing Requests</h5>
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2 font-medium" x-text="item.institute_name"></td>
                                <td class="px-4 py-2 text-sm" x-text="item.owner_name"></td>
                                <td class="px-4 py-2 text-sm" x-text="item.mobile"></td>
                                <td class="px-4 py-2 text-sm" x-text="item.city"></td>
                                <td class="px-4 py-2 text-sm" x-text="item.category || '-'"></td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/listing-requests') }}/${item.id}`" class="btn btn-sm btn-outline-primary">View</a>
                        ...
                        <tr x-show="items.length === 0">
                            <td colspan="9" class="px-4 py-4 text-center text-gray-500">No listing requests found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('listingList', () => ({
                items: @json($requests->items()),
                pagination: {
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.listing-requests.index') }}?page=${page}`;
                ...
                deleteItem(id) {
                    ...
                    swal.fire({ title: 'Delete request?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/listing-requests') }}/${id}`, {
                                method: 'DELETE',
