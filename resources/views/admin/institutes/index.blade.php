<x-layout.admin title="Institutes">
    <div x-data="instituteList">
        <div class="flex items-center justify-between gap-4 mb-5">
            <h5 class="text-lg font-semibold dark:text-white-light">Institutes</h5>
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <select class="form-select py-2 w-40" x-model="filterCity" @change="fetchData(1)">
                    <option value="">All Cities</option>
                    @foreach($cities as $city)
                        <option value="{{ $city->id }}">{{ $city->name }}</option>
                    @endforeach
                </select>
                <select class="form-select py-2 w-48" x-model="filterCategory" @change="fetchData(1)">
                ...
                <button type="button" class="btn btn-outline-danger btn-sm"
                    x-show="searchText || filterCity || filterCategory || filterStatus"
                    @click="clearFilters()">Clear</button>
                <a href="{{ route('admin.institutes.create') }}" class="btn btn-primary gap-2 whitespace-nowrap">
                ...
                <table class="table-hover">
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="item.id">
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    <div class="text-xs text-gray-400" x-text="item.phone || ''"></div>
                                </td>
                                <td class="px-4 py-2 text-sm" x-text="item.category ? item.category.name : '-'"></td>
                                <td class="px-4 py-2 text-sm" x-text="item.city ? item.city.name : '-'"></td>
                                <td class="px-4 py-2">
                                    <span class="badge cursor-pointer"
                                        :class="item.status === 'active' ? 'bg-success' : item.status === 'inactive' ? 'bg-danger' : 'bg-warning'"
                                        x-text="item.status"
                                        @click="toggleStatus(item.id)"></span>
                                </td>
                                <td class="px-4 py-2">
                                    <span class="badge cursor-pointer"
                                        :class="item.is_verified ? 'bg-info' : 'bg-gray-400'"
                                        x-text="item.is_verified ? 'Verified' : 'Unverified'"
                                        @click="toggleVerified(item.id)"></span>
                                </td>
                                <td class="px-4 py-2">
                                    <span class="badge cursor-pointer"
                                        :class="item.is_featured ? 'bg-warning' : 'bg-gray-400'"
                                        x-text="item.is_featured ? 'Featured' : 'Normal'"
                                        @click="toggleFeatured(item.id)"></span>
                                </td>
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="`{{ url('admin/institutes') }}/${item.id}/edit`" class="btn btn-sm btn-outline-primary">Edit</a>
                        ...
                        <tr x-show="items.length === 0">
                            <td colspan="8" class="px-4 py-4 text-center text-gray-500">No institutes found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data('instituteList', () => ({
                items: @json($institutes->items()),
                pagination: {
                ...
                fetchData(page = 1) {
                    let url = `{{ route('admin.institutes.index') }}?page=${page}`;
                ...
                clearFilters() {
                    this.searchText = ''; this.filterCity = ''; this.filterCategory = ''; this.filterStatus = '';
                    this.fetchData(1);
                },

                toggleStatus(id) {
                    fetch(`{{ url('admin/institutes') }}/${id}/toggle-status`, {
                        method: 'PATCH',
                ...
                toggleFeatured(id) {
                    fetch(`{{ url('admin/institutes') }}/${id}/toggle-featured`, {
                        method: 'PATCH',
                        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                    }).then(res => res.json()).then(data => {
                        if (data.success) { this.showMessage(data.message); this.fetchData(this.pagination.current_page); }
                ...
                toggleVerified(id) {
                    fetch(`{{ url('admin/institutes') }}/${id}/toggle-verified`, {
                        method: 'PATCH',
                        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                    }).then(res => res.json()).then(data => {
                        if (data.success) { this.showMessage(data.message); this.fetchData(this.pagination.current_page); }
                ...
                deleteItem(id) {
                    ...
                    swal.fire({ title: 'Are you sure?', text: 'This will soft-delete the institute!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', cancelButtonText: 'Cancel', reverseButtons: true, padding: '2em' }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`{{ url('admin/institutes') }}/${id}`, {
                                method: 'DELETE',
                                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }
                            }).then(res => res.json()).then(data => {
                                if (data.success) { this.showMessage(data.message); this.fetchData(this.pagination.current_page); }
